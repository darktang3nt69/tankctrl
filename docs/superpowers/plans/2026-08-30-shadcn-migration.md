...
- **shadcn CLI config** (set in Task 2): style `radix-nova` (the CLI's current name for what this plan originally called "New York" — Nova is its renamed successor, confirmed during Task 2's review), base color `neutral`, CSS variables enabled — Task 3 overwrites the generated tokens with the project's own amber/neutral palette.
